...
export class AgendaRow extends Component {
  ...
  async getUserData() {
    ...
    const data = await response.json().then((res) => {
      return typeof res.error == "string" ? false : res;
    });
    ...
    if (data === false) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (item) {
          ...
          // de id die wordt gebruikt in de url
          id = item.ID;
        }
    ...
    return (
      <tr>
        ...
        {this.state.cells.map((cell) => (
          <td
            key={cell.ts}
            style={{ backgroundColor: this.getCellColour(cell) }}
          >
            <a
              href={`agenda?modal=1&id=${cell.id}&user=${this.state.user}&ts=${cell.ts}`}
            >
              {cell.status ?? "&nbsp;"}
            </a>
          </td>
